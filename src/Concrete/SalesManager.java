...
public class SalesManager implements SalesService {
	...
	public void buy(Order order, Customer customer, Game[] games, Campaign campaign) {
		...
		for (Game game : games) {
			System.out.println("Game ID : " + game.getId() + " --> " + game.getName() + " oyunun fiyatý : "
					+ game.getPrice() + "$");
			totalPrice += game.getPrice();
		}
		System.out.println("\n\tToplam sepet tutarýnýz : " + totalPrice + "$\n");
	...
	private void calculate(Campaign campaign, Double totalPrice) {
		System.out.println(campaign.getName() + " kampanyasýndan %" + campaign.getDiscount() + " indirim sayesinde "
				+ totalPrice + "$ fiyatýndan güncel sepet tutarýnýz : --> "
